...
static bool
isRecordSymbolValid(Symbol* sym) {
  //
  ...
  for_defs(se, defMap, sym) {
    if (se->parentSymbol) {
	  ...
	  if(!call->isPrimitive(PRIM_MOVE))
		return false;
	  if(!isSymExpr(call->get(2)) && !toCallExpr(call->get(2))->isPrimitive(PRIM_GET_MEMBER_VALUE))
		return false;
    }
  }
  ...
  for_uses(se, useMap, sym) {
    if (se->parentSymbol) {
      ...
      if (!call)
        ...
        return false;
	  if(!(call->isPrimitive(PRIM_SET_MEMBER) && call->get(1) == se) &&
		  !call->isPrimitive(PRIM_GET_MEMBER) &&
		  !call->isPrimitive(PRIM_GET_MEMBER_VALUE) &&
...
static SymbolMap
generateRecordFieldMap(AggregateType* ct, Symbol* sym) {
  //
  // okay, let's scalar replace sym; first, create vars for every
  // field in sym and compute fieldMap to map the fields to these new
  // vars
...
static void
scalarReplaceRecordDefs(AggregateType* ct, Symbol* sym, SymbolMap* fieldMap) {
  //
...
static void
scalarReplaceRecordUses(AggregateType* ct, Symbol* sym, SymbolMap* fieldMap) {
  //
  // replace uses of sym with new vars
  //
  for_uses(se, useMap, sym) {
...
scalarReplaceRecord(AggregateType* ct, Symbol* sym) {
  ...
  if(!isRecordSymbolValid(sym)) return false;

  if (fReportScalarReplace) srRecordReplaced++;
  
  SymbolMap fieldMap = generateRecordFieldMap(ct, sym);
  
  scalarReplaceRecordDefs(ct, sym, fieldMap);
  ...
  scalarReplaceRecordUses(ct, sym, fieldMap);
